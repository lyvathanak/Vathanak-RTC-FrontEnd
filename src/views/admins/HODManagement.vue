<template>
  <div class="flex flex-col gap-4 py-3 sm:py-5">
    ...
    <!-- View HOD Modal -->
    <ViewHODModal v-model="showViewModal" :hod="viewHOD" />
  </div>
</template>
...
const { departments, loading: deptsLoading } = useFilteredByDepartment({
  immediate: true,
});
...
function normalizeRow(u) {
  ...
  const department_name =
    ...
    (typeof u.department === "string" && u.department.trim()
      ? u.department
      : "") ||
...
const filteredRows = computed(() => {
  ...
  if (filters.value.gender !== "All") {
    list = list.filter((r) => {
      const g =
        r.gender === "M" ? "Male" : r.gender === "F" ? "Female" : r.gender;
