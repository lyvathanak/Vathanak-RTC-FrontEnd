...
const fetchLeaveRequests = async () => {
  ...
  try {
    ...
    const {
      ...
      pagination,
      error: apiError,
    } = await apiLeave.getAllLeaveRequests(params);
...
const applyClientSideFilters = (requests) => {
  ...
  if (
    ...
    filters.value.department !== "Loading..."
  ) {
    filtered = filtered.filter((req) => {
      const dept =
        req.originalData?.department || req.originalData?.department_name;
...
// Check if leave request is within 2 days from now
const isUrgent = (leaveRequest) => {
  const data = leaveRequest.originalData;

  const startDateStr =
    data?.start_date || data?.from_date || data?.leave_from || data?.date_from;
...
const urgentCount = computed(() => {
  return allLeaveRequests.value.filter(isUrgent).length;
});
