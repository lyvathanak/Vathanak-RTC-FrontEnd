          ...
          try {
            ...
            if (groupDetail.success && groupDetail.data) {
              return {
                ...
                student_count: groupDetail.data.students
                  ? groupDetail.data.students.length
                  : 0,
              };
            }
            return group;
...
const filteredGroups = computed(() => {
  ...
  if (filters.value.search?.trim()) {
    ...
    filtered = filtered.filter((group) => {
      try {
        ...
        const department =
          getDepartmentById(group.department_id) ||
          getDepartmentById(group.dept_id);
        const program =
          getProgramById(group.program_id) ||
          getProgramById(group.programme_id);
        const section =
          getSectionById(group.section_id) ||
        ...
        const departmentName =
          ...
          "";
        const programName =
          program?.program_name || group.program || group.program_name || "";
        const sectionName =
          section?.name || group.section || group.section_name || "";
  ...
  if (filters.value.shift) {
    filtered = filtered.filter((group) => group.shift === filters.value.shift);
  }
...
const handleUpdateGroup = async (updatedGroup) => {
  ...
  try {
    ...
    if (result.success) {
      ...
      showNotification(
        `Group "${
          updatedGroup.name ?? updatedGroup.group_name
