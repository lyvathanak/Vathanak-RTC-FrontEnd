...
import { ChevronDown, CalendarDaysIcon } from "lucide-vue-next"; // ⟵ added for custom dropdown arrows
...
function moveDraftToWeekday(dayIdx) {
  ...
  s.setHours(
    draft.value.start.getHours(),
    draft.value.start.getMinutes(),
    0,
    0
  );
...
const groupOptions = computed(() =>
  filterSemesterId.value == null
    ? []
    : allGroupOptions.value.filter(
        (g) => g.semester_id === filterSemesterId.value
      )
);
...
function ensureMinDuration(start, end) {
  if ((+end - +start) / 60000 < MIN_MIN) {
    ...
    e.setMinutes(e.getMinutes() + MIN_MIN);
    return e;
  }
  return end;
}
...
function parseAPIDate(d) {
  ...
  if ((d || "").length >= 10 && String(a).length === 4)
    return { y: a || 1970, m: b || 1, d: c || 1 };
...
function widenCalendarWindow(evts) {
  if (!evts.length) {
    timeFromM.value = BASE_FROM;
    timeToM.value = BASE_TO;
    return;
  }
  let minM = 24 * 60,
    maxM = 0;
  for (const e of evts) {
    minM = Math.min(minM, minutesOf(e.start));
    maxM = Math.max(maxM, minutesOf(e.end));
  }
  const suggestedFrom = Math.max(0, Math.floor(minM / 60 - 1) * 60);
  const suggestedTo = Math.min(
    24 * 60,
    Math.ceil(maxM / 60 + EXTRA_HOURS_AFTER_LAST) * 60
  );
...
function buildEventsFromSlots(slots) {
  ...
  return slots.map((s) => {
    let start_time = "08:00:00",
      end_time = "09:00:00";
    try {
      const j = JSON.parse(s.time_slot || "{}");
      start_time = j.start_time || start_time;
      end_time = j.end_time || end_time;
    } catch {}
...
async function jumpTo(dateLike) {
  ...
  await nextTick();
  try {
    cal.value?.switchView?.("week", new Date(d));
  } catch {}
  await forceLayout();
}
...
async function refreshTimeTables() {
  ...
  if (haveGroupSelected) {
    const gName =
      groupOptions.value.find((g) => g.id === filterGroupId.value)?.name ||
      "Group";
    const sName =
      semesterOptions.value.find((s) => s.id === filterSemesterId.value)
        ?.name || "Semester";
    const pName =
      programOptions.value.find((p) => p.id === selectedProgramId.value)
        ?.name || "Program";

    const name = `Time Table • ${gName}`;
    const desc = `Auto-created for ${pName} / ${sName}`;

    const created = await apiCreateTimeTable({
      name,
      description: desc,
    ...
    if (created.success) {
      ...
      if (after.success && after.data.length) {
        ...
        programmaticTTChange.value = false;
        showNotification(
          "No timetable found — created an empty one for you.",
          "success"
        );
        return;
      }
...
function eventsInWeek(events, weekStart) {
  ...
  return events.filter(
    (e) =>
      e.timeTableId === selectedTimeTableId.value &&
      e.start >= weekStart &&
      e.start < weekEnd
  );
}
const filteredEvents = computed(() =>
  allEvents.value.filter((e) => e.timeTableId === selectedTimeTableId.value)
);
const weekEvents = computed(() =>
  eventsInWeek(filteredEvents.value, currentWeekStart.value)
);
const weekIsEmpty = computed(
  () => !!selectedTimeTableId.value && weekEvents.value.length === 0
);
...
async function maybeFetchWithFilters() {
  if (!filtersActive.value || !calendarEnabled.value) {
    allEvents.value = [];
    return;
  }
  ...
  try {
    ...
    if (
      String(e?.message || "")
        .toLowerCase()
        .includes("no time slots available")
    ) {
      allEvents.value = [];
      await refreshLayoutKeepFocus();
      return;
...
/* --------------------------------- CRUD glue -------------------------------- */
async function createSlotOnServer(tableId, ev) {
  return await createSlot(tableId, eventToSlot(ev, subjectId));
}
async function updateSlotOnServer(slotId, ev) {
  return await updateSlot(slotId, eventToSlot(ev, subjectId));
}
async function deleteSlotOnServer(tableId, slotId) {
  return await removeSlots(tableId, [slotId]);
}

/* ------------------------------- UI Handlers ------------------------------- */
function eventTooltip(ev) {
  const date = ev.start.toLocaleDateString(undefined, {
    weekday: "long",
    month: "short",
    day: "numeric",
    year: "numeric",
  });
  const t1 = ev.start.toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  });
  const t2 = ev.end.toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  });
  return `${ev.title}\n${date} • ${t1} – ${t2}\nTeacher: ${
...
const draftDayLabel = computed(() =>
  draft.value
    ? draft.value.start.toLocaleDateString(undefined, {
        weekday: "long",
        month: "short",
        day: "numeric",
      })
...
function openCreateDialog(startApprox) {
  if (!selectedTimeTableId.value) {
    showNotification("No time table resolved from filters.", "error");
    return;
  }
  formError.value = null;
  isEditing.value = false;
  editingSlotId.value = null;
  const s = new Date(startApprox);
  const e = new Date(startApprox);
  e.setMinutes(e.getMinutes() + MIN_MIN);
  draft.value = {
  ...
  draftStart.value = `${String(s.getHours()).padStart(2, "0")}:${String(
    s.getMinutes()
  ).padStart(2, "0")}`;
  draftEnd.value = `${String(e.getHours()).padStart(2, "0")}:${String(
    e.getMinutes()
  ).padStart(2, "0")}`;
...
function openEditFromEvent(ev) {
  ...
  editingSlotId.value = Number(ev.id);
...
function applyDraftToEvent() {
  ...
  if (draftTeacherId.value !== "")
    draft.value.teacherId = Number(draftTeacherId.value);
  if (draftRoomId.value !== "") draft.value.roomId = Number(draftRoomId.value);
  draft.value.teacherName =
    (
      staffOptions.value.find(
        (t) => Number(t.id) === Number(draft.value.teacherId)
      ) || {}
    ).name || draft.value.teacherName;
  draft.value.roomName =
    (
      roomOptions.value.find(
        (r) => Number(r.id) === Number(draft.value.roomId)
      ) || {}
    ).name || draft.value.roomName;
}
async function saveEditor() {
  if (!draft.value || !selectedTimeTableId.value) {
    showNotification("No time table resolved from filters.", "error");
    return;
  }
  applyDraftToEvent();
  const roomOk =
    Number.isFinite(Number(draft.value.roomId)) &&
    Number(draft.value.roomId) > 0;
  if (!roomOk) {
    formError.value = "Please choose a room.";
    return;
  }
  ...
  try {
    if (isEditing.value && editingSlotId.value != null) {
      ...
      replaceEventLocal({
        ...(allEvents.value.find(
          (e) => String(e.id) === String(editingSlotId.value)
        ) || {}),
        ...draft.value,
      ...
      const res = await createSlotOnServer(
      ...
      if (created.length) {
        created.forEach((ev) =>
          replaceEventLocal({
            ...ev,
            timeTableId: ev.timeTableId ?? selectedTimeTableId.value,
          })
        );
      } else {
        replaceEventLocal({
...
async function onEventDelete(ev) {
  ...
  const closeEditorAfter =
    showEditor.value && Number(editingSlotId.value) === Number(ev.id);

  desiredFocusDate.value = new Date(currentWeekStart.value);
  try {
    const res = await deleteSlotOnServer(
      selectedTimeTableId.value,
      Number(ev.id)
    );
    if (!res.success) throw res.error;
...
async function cloneFromWeek(sourceWeekStart) {
  ...
  if (!src.length) {
    showNotification("Source week has no events.", "error");
    return;
  }

  const deltaDays = Math.round(
    (currentWeekStart.value - sourceWeekStart) / (1000 * 60 * 60 * 24)
  );
  ...
  try {
    for (const ev of src) {
      ...
      const res = await createSlotOnServer(selectedTimeTableId.value, {
        ...ev,
        start: s,
        end: e,
      });
      if (res.success) {
        ...
        if (newEvents.length) {
          newEvents.forEach((ne) =>
            replaceEventLocal({
              ...ne,
              timeTableId: ne.timeTableId ?? selectedTimeTableId.value,
            })
          );
        }
        created++;
        ...
        /time conflict/i.test(res.error?.message || "")
      ) {
        conflicts++;
      } else {
        failed++;
    ...
    showNotification(
      parts.join(", ") || "Nothing cloned.",
      conflicts || failed ? "warning" : "success"
    );
...
async function cloneFromLastWeek() {
  await cloneFromWeek(addDays(currentWeekStart.value, -7));
}
async function cloneFromCustom() {
  await cloneFromWeek(
    isoWeekStart(Number(customCloneYear.value), Number(customCloneWeek.value))
  );
}
...
async function createTimeTableUI(name, description, groupId) {
  return await apiCreateTimeTable({ name, description, group_id: groupId });
}
async function deleteTimeTableUI(tableId) {
  return await apiDeleteTimeTable(tableId);
}
async function doCreateTable() {
  if (!filterGroupId.value) {
    showNotification("Pick a group first.", "error");
    return;
  }
  const res = await createTimeTableUI(
    newTableName.value.trim(),
    newTableDesc.value.trim(),
    Number(filterGroupId.value)
  );
  if (!res.success) {
    showNotification(
      res.error?.message || "Failed to create time table",
      "error"
    );
    return;
  }
...
async function doDeleteTable() {
  ...
  if (!id) {
    showNotification("No time table selected.", "error");
    return;
  }
  const res = await deleteTimeTableUI(id);
  if (!res.success) {
    showNotification(
      res.error?.message || "Failed to delete time table",
      "error"
    );
    return;
  }
