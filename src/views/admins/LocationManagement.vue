<template>
  <div
    ...
    <div
      ...
      <div class="flex flex-col sm:flex-row gap-2 sm:gap-3">
        <button
          ...
          class="h-10 inline-flex items-center justify-center gap-2 px-3 sm:px-4 py-2 bg-[#235AA6] text-white rounded-lg hover:bg-[#1E4A78] transition-colors text-sm">
          <Plus class="w-4 h-4" />
    ...
    <div v-if="loading" class="text-center py-8 sm:py-12">
      <div
        class="animate-spin rounded-full h-6 w-6 sm:h-8 sm:w-8 border-b-2 border-blue-600 mx-auto mb-3 sm:mb-4"></div>
      <p
        :class="[
          locale === 'kh'
            ? 'khmer-text text-sm sm:text-base'
...
const initializeData = async () => {
  await Promise.all([loadBuildings(), loadLocations()]);
};
...
const handleBuildingSubmit = async ({ name, mode }) => {
  try {
    ...
    if (mode === "create") {
      ...
      result = await LocationCRUD.updateBuilding(
...
const handleLocationSubmit = async ({ mode, payload }) => {
  try {
    ...
    if (mode === "create") {
      ...
      result = await LocationCRUD.updateLocation(
...
const getFloorsForBuilding = (buildingId) => {
  const buildingLocations = locations.value.filter(
    (loc) => loc.building_id === buildingId
  );
...
const getFilteredLocations = (buildingId) => {
  let buildingLocations = locations.value.filter(
    (loc) => loc.building_id === buildingId
  );

  if (locationFilters.value.floor !== "All") {
    buildingLocations = buildingLocations.filter(
      (loc) => loc.floor === locationFilters.value.floor
    );
  }
