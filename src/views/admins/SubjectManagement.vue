    ...
    <div
      ...
      <div class="flex items-center gap-2">
        <button
          ...
          class="h-10 shrink-0 whitespace-nowrap inline-flex items-center gap-2 rounded-lg bg-[#235AA6] px-4 text-white font-semibold hover:bg-[#1f4f93] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#235AA6]">
          <Plus class="h-4 w-4" />
        ...
        <button
          ...
          class="h-10 shrink-0 whitespace-nowrap inline-flex items-center gap-2 rounded-lg bg-orange-500 px-4 text-white font-semibold hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 disabled:opacity-60">
          <span
            v-if="exporting"
            class="inline-block h-4 w-4 border-2 border-white/80 border-t-transparent rounded-full animate-spin" />
    ...
    <div class="px-3 sm:px-5 flex flex-col md:flex-row md:items-center gap-3">
      ...
      <div class="relative w-full md:w-[20rem]">
        <select
          ...
          <option
            v-for="d in departmentsWithPrograms"
            :key="d.id"
            :value="String(d.id)">
            {{ d.name }}
          </option>
      ...
      <transition name="fade">
        <div v-if="filters.department_id" class="relative w-full md:w-[20rem]">
          <select
            ...
            <option
              v-for="p in filteredProgramOptions"
              :key="p.id"
              :value="String(p.id)">
              {{ p.name }}
            </option>
        ...
        <table class="w-full subject-table">
          <thead class="bg-gray-50 border-b border-gray-200">
            <tr class="text-xs font-semibold text-gray-600 uppercase">
              ...
              <th
                class="px-3 py-3 text-left sticky-right w-[88px] min-w-[88px] md:w-[96px] md:min-w-[96px]">
                Action
              </th>
            </tr>
          </thead>
          ...
          <tbody class="divide-y">
            <tr v-for="s in subjects" :key="s.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3 td-name">
                ...
                <div
                  v-if="s.description"
                  class="desc-ellipsis text-xs text-gray-500 mt-0.5"
                  :title="s.description">
                  {{ s.description }}
                </div>
              </td>
...
async function fetchSubjects(p = 1) {
  ...
  try {
    ...
    const normalized = rows.map((r) => {
      ...
      const name =
        r.subject_name ?? r.name ?? r.title ?? r.subject ?? r.subjectName;
      const totalHours =
...
function onUpdated(updated) {
  if (updated?.id) {
    const i = subjects.value.findIndex(
      (r) => Number(r.id) === Number(updated.id)
    );
    if (i !== -1) {
      subjects.value[i] = { ...subjects.value[i], ...updated };
      subjects.value[i].__name =
        updated.subject_name ?? subjects.value[i].__name;
      subjects.value[i].__code =
        updated.subject_code ?? subjects.value[i].__code;
    }
  }
