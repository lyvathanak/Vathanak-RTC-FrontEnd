<template>
  <div
    ...
    <div
      ...
      <div
        class="relative w-full max-w-3xl bg-white rounded-lg shadow-lg max-h-[90vh] overflow-y-auto">
        <LeaveRequestTeacherForm
...
import ViewLeaveRequestForm from "@/components/teachers/ViewLeaveRequestForm.vue";
import LeaveRequestTeacherForm from "@/components/features/LeaveRequestTeacherForm.vue";
...
const totalFilteredItems = computed(() => {
  // Re-apply filters without pagination to get total count
  let filtered = [...allLeaveRequests.value];
...
const fetchLeaveRequests = async () => {
  ...
  try {
    const {
      ...
      requests,
      error: apiError,
    } = await getAllLeaveRequestsByTeacher();

    if (success) {
      allLeaveRequests.value = requests.map(transformLeaveRequestData);
    } else {
      console.error("Failed to fetch leave requests:", apiError);
