...
// Get method and contact info from query params
...
async function resend() {
  ...
  try {
    if (method === 'phone') {
      ...
      await resendOtpByPhone(phone)
      message.value = { 
        type: 'success', 
        text: t('otp_sent_again') || 'OTP sent to your Telegram again.' 
      }
    } else {
      if (!email) {
      ...
      message.value = { 
        type: 'success', 
        text: t('otp_sent_again') || 'OTP sent to your email again.' 
      }
    }
    startTimer()
  } catch (e) {
    message.value = { 
      type: 'error', 
      text: (e && e.message) || t('an_unexpected_error_occurred') || 'Error. Try again.' 
    }
  }
}
...
async function onSubmit() {
  ...
  try {
    ...
    message.value = { type: 'success', text: t('otp_verified') || 'OTP verified successfully!' }
    
    // Get reset_token from backend response (backend generates new token after OTP verification)
    const resetToken = result.reset_token || code
    
    if (!resetToken) {
      throw new Error('Failed to get reset token. Please try again.')
    }
    
    // Wait a moment to show success message, then route to reset password page
    setTimeout(() => {
      const currentLang = router.currentRoute.value.params.lang || 'en'
      router.push({
        path: `/${currentLang}/confirm-password`,
        query: {
...
/* ------- Real API calls ------- */
async function resendOtpByPhone(phoneNumber) {
...
async function resendOtpByEmail(emailAddress) {
  // Placeholder for email OTP resend
