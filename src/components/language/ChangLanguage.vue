  ...
  <div class="relative inline-block text-left" data-language-dropdown>
    ...
    <button
      ...
      <span
        :class="[
          'text-sm font-medium',
          current.code === 'kh' ? 'khmer-text' : '',
        ]"
        >{{ current.label }}</span
      >
      <ChevronDown class="w-4 h-4 text-whtie" />
    </button>
    ...
    <transition name="fade">
      <ul
        v-if="open"
        @click.stop
        class="absolute right-0 mt-2 w-36 bg-white rounded-md shadow-lg z-100 text-black">
        <li
          v-for="lang in languages"
          :key="lang.code"
          @click="select(lang)"
          class="flex items-center px-3 py-2 hover:bg-gray-200 rounded-lg cursor-pointer space-x-2">
          <img
            :src="lang.flag"
            :alt="lang.label"
            class="w-8 rounded object-cover" />
          <span :class="['text-sm', lang.code === 'kh' ? 'khmer-text' : '']">{{
            lang.label
          }}</span>
        </li>
      </ul>
...
onMounted(() => {
  ...
  if (
    routeLang &&
    ["en", "fr", "kh"].includes(routeLang) &&
    routeLang !== locale.value
  ) {
