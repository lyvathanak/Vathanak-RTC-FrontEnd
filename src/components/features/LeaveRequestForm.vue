    ...
    <div
      ...
      <form @submit.prevent="submitLeaveRequest" class="space-y-4">
        ...
        <div class="flex justify-center mt-6">
          <button
            type="submit"
            class="bg-blue-600 text-white px-6 py-2 rounded-[10px] w-full sm:w-[250px] hover:bg-blue-700 transition">
...
const submitLeaveRequest = async () => {
  try {
    ...
    formData.append("user_id", form.id); // ✅ correct
    formData.append("type", form.leaveType);
    formData.append("start_day", form.startDate); // ✅ correct
    formData.append("end_day", form.endDate); // ✅ correct
    formData.append("reason", form.reason);
    formData.append("handover_details", form.handover); // ✅ plural
    ...
    if (error.response?.data?.errors) {
      ...
      showNotification("Failed to submit leave request.");
    }
  }
};
watch(
