<template>
  <div
    ...
    <div class="flex items-center gap-1 sm:gap-2 order-1 sm:order-2">
      ...
      <!-- Page Size Selector -->
      <div class="flex items-center gap-1 sm:gap-2 order-1 sm:order-2">
        <div v-if="showPageSizeSelector" class="relative ml-1 sm:ml-2">
          <select
            :value="pageSize"
            @change="changePageSize($event.target.value)"
            class="rounded-lg border border-gray-300 text-xs sm:text-sm px-2 py-1.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 min-w-16 sm:min-w-20 appearance-none pr-6 cursor-pointer h-8 sm:h-9"
            :title="pageSizeTitle">
          ...
          <div
            class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-1.5">
            <ChevronDown :size="16" class="text-gray-700" />
          </div>
        </div>
      </div>
    </div>
