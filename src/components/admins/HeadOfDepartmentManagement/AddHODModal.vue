    ...
    <div v-if="showAdd" class="fixed inset-0 z-50">
      ...
      <div
        ...
        <div
          role="dialog"
          aria-modal="true"
          class="w-full max-w-xs sm:max-w-lg md:max-w-2xl lg:max-w-4xl h-[90vh] sm:h-[85vh] rounded-xl bg-white shadow-xl overflow-hidden border border-gray-200 flex flex-col"
          @click.stop>
          <!-- header -->
          <div
          ...
          <!-- body -->
          <div
            class="px-4 sm:px-5 md:px-6 py-4 sm:py-5 flex-1 overflow-y-auto min-h-0">
            <div class="flex flex-col gap-4 sm:gap-6">
              ...
              <div class="w-full space-y-4 sm:space-y-6">
                ...
                <section>
                  <div
                    class="flex items-center font-semibold mb-3 text-sm sm:text-base">
                ...
                <section>
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    ...
                    <div>
                      ...
                      <div
                        v-else-if="deptErrorString"
                        class="text-sm text-red-600 py-2">
                        {{ deptErrorString }}
                        <button class="ml-2 underline" @click="reloadDepts">
                          Retry
                        </button>
                      </div>

                      <div
                        v-else-if="deptOptions.length === 0"
                        class="text-sm text-gray-500 py-2">
                        No available departments found. All departments may
                        already have heads assigned.
                        <button class="underline" @click="reloadDepts">
                          Reload
                        </button>
                      </div>
          ...
          <!-- footer -->
          <div
            class="px-4 sm:px-5 md:px-6 py-3 sm:py-4 border-t bg-gray-50 flex justify-end gap-2 sm:gap-3">
            <button
              class="px-3 sm:px-4 py-2 text-sm rounded-lg bg-[#FF4040] text-white transition-colors hover:bg-red-700"
              @click="closeAdd">
              Cancel
            </button>
            <button
              class="px-3 sm:px-4 py-2 text-sm rounded-lg bg-[#235AA6] text-white hover:bg-[#1e4a94] transition-colors"
              @click="saveHOD">
              Save
            </button>
          </div>
...
const deptErrorString = computed(() => {
  const e =
    typeof dept.error === "object" && dept.error && "value" in dept.error
      ? dept.error.value
      : dept.error;
  if (!e) return "";
...
const allDeptOptions = computed(() => {
  const opts =
    typeof dept.departmentOptions === "object" &&
    dept.departmentOptions &&
    "value" in dept.departmentOptions
...
async function loadAllHODs() {
  try {
    ...
    console.log(
      "🏢 Loaded HODs for department filtering:",
      allHODs.value.length
    );
  } catch (error) {
...
function saveHOD() {
  ...
  const hodData = {
    ...
    profile_picture: newHOD.file
      ? undefined
      : newHOD.profile_picture || undefined, // string URL if no file
  };
