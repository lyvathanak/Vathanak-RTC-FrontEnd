                ...
                <div v-show="basicOpen" class="mt-3 border rounded-lg p-4">
                  <div class="grid grid-cols-1 md:grid-cols-[160px_1fr] gap-4">
                    ...
                    <div class="flex flex-col items-center">
                      <div
                        class="w-32 h-40 bg-gray-100 border rounded-lg overflow-hidden mb-2 cursor-pointer hover:ring-2 hover:ring-blue-400"
                        @click="openImageViewer">
                        <img
                          v-if="currentImageSrc"
                          :src="currentImageSrc"
                          :alt="
                            hod?.full_name || hod?.latin_name || 'HOD Photo'
                          "
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div class="md:col-span-2">
                        ...
                        <textarea
                          v-model="form.bio"
                          rows="3"
                          class="w-full rounded-md border px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500 resize-none"></textarea>
                      </div>
                    </div>
                  </div>
                </div>
...
watch(
  ...
  () => {
    ...
    form.value = {
      ...
      department:
        ...
        (Number.isFinite(depId)
          ? (props.departments || []).find((d) => Number(d.id) === depId)
              ?.department_name
          : "") ||
        "",
    ...
    if (Number.isFinite(depId)) setDepartment(depId);
    else setDepartment("");
  },
  { immediate: true }
);
