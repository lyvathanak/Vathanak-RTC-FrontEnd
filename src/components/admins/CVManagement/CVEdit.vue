<template>
  ...
  <div
    ...
    <div
        ...
        <div class="min-w-0">
          <h3
            ...
            <span
              ...
              <Pencil class="w-5 h-5 text-blue-700" />
            </span>
            Edit CV Submission
          </h3>

          <p class="text-sm text-gray-600 mt-1 truncate">
            {{ localCv?.template_name || "—" }}
          </p>
        </div>

        <button
          @click="emit('close')"
          class="h-10 w-10 inline-flex items-center justify-center rounded-lg border border-gray-200 hover:bg-gray-50 transition"
          aria-label="Close"
          type="button">
        ...
        <div class="mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div
            ...
            <p class="mt-1 font-semibold text-gray-900">
              {{ formatSubmittedDate(localCv?.submitted_date) }}
            </p>
          </div>
          ...
          <div
            ...
            <span
              :class="[
                ...
                statusPillClass(localCv?.status),
              ]">
              <component :is="statusIcon(localCv?.status)" class="w-4 h-4" />
              {{ statusLabel(localCv?.status) }}
            </span>
          </div>
        </div>
              ...
              <div
                ...
                <div class="px-4 pt-4 pb-3 border-b border-gray-100">
                  ...
                  <div class="mt-3 relative">
                    <input
                      ...
                      class="w-full h-10 rounded-lg border border-gray-200 bg-gray-50 px-3 pl-9 text-sm font-semibold text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-300"
                ...
                <div class="p-2 space-y-1">
                  <button
                    ...
                    class="w-full group relative flex items-center justify-between rounded-lg px-3 py-2.5 text-sm font-semibold transition"
                    :class="
                ...
                <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div
                    ...
                    <div class="flex items-start justify-between gap-3">
                      ...
                      <button
                        v-if="
                          !isFileField(localCv.form_data[field.key]) &&
                          localCv.form_data[field.key]
                        "
                        @click="copyValue(localCv.form_data[field.key])"
                    ...
                    <!-- FILE: keep same preview UI (read-only) -->
                    <div
                      v-if="isFileField(localCv.form_data[field.key])"
                      class="mt-3">
                      <button
                        ...
                        <div class="flex items-center gap-3">
                          <div
                            ...
                            <FileText
                              v-if="!isImageFile(localCv.form_data[field.key])"
                      ...
                      <p class="text-[11px] font-semibold text-gray-500 mt-2">
                        File editing not included — click to preview / download
                      </p>
                    </div>

                    <!-- LONG TEXT -->
                    <div
                      v-else-if="isLongText(localCv.form_data[field.key])"
                      class="mt-3">
                      <textarea
                        v-model="localCv.form_data[field.key]"
                        rows="5"
                        class="w-full rounded-2xl bg-gray-50 border border-gray-200 p-3 text-sm font-semibold text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-300"
                    ...
                    <div v-else class="mt-3">
                      <input
                        v-model="localCv.form_data[field.key]"
                        type="text"
                        class="w-full h-11 rounded-2xl bg-gray-50 border border-gray-200 px-3 text-sm font-semibold text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-300"
                        placeholder="Enter value..." />
                    </div>
                  </div>
                </div>
      ...
      <div
        ...
        <button
          @click="emit('close')"
          class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-red-700 px-4 py-2 text-sm font-semibold text-white hover:bg-red-800 transition"
          type="button">
          Cancel
        </button>
        ...
        <button
          @click="save"
          class="inline-flex items-center gap-2 rounded-lg bg-[#235AA6] px-4 py-2 text-sm font-semibold text-white hover:bg-[#1E4A78] transition"
          type="button">
          <Save class="w-4 h-4" />
          Save Changes
        </button>
  ...
  <!-- File Preview Modal (same as CVDetail) -->
  <div
    ...
    <div
      ...
      <div
        ...
        <button
          @click="showFilePreview = false"
          class="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-300 bg-white text-gray-700 font-semibold hover:bg-gray-50"
          type="button">
          Close
        </button>
      </div>
...
const localCv = ref(null);

/* clone on open/change */
watch(
  () => props.cv,
  (v) => {
    if (!v) {
    ...
    original.value = JSON.parse(JSON.stringify(v));
    localCv.value = JSON.parse(JSON.stringify(v));
  },
  { immediate: true },
);

const hasFormData = computed(() => {
  const data = localCv.value?.form_data;
  return !!data && Object.keys(data).length > 0;
});

/* Status helpers (same as CVDetail) */
...
/* File helpers (same as CVDetail, but use localCv) */
...
const getFileName = (fieldKey) => {
  const value = localCv.value?.form_data?.[fieldKey];
  if (!value) return "—";
...
/* Group fields (same logic as CVDetail but from localCv) */
const groupedFields = computed(() => {
  if (!localCv.value?.form_data) return {};
  ...
  Object.keys(localCv.value.form_data).forEach((key) => {
...
const resetChanges = () => {
  ...
  localCv.value = JSON.parse(JSON.stringify(original.value));
};

const save = () => {
  if (!localCv.value) return;
...
const onKeydown = (e) => {
  ...
  if (showFilePreview.value) showFilePreview.value = false;
...
watch(
  () => props.open || showFilePreview.value,
  (isOpen) => {
