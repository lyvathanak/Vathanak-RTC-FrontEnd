    ...
    <div
      ...
      <div
          ...
          <section>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="md:col-span-2">
                ...
                <div v-if="!editMode" class="text-sm text-gray-800">
                  {{ dept?.description || "—" }}
                </div>
                <textarea
                  v-else
          ...
          <section>
            ...
            <div class="mb-3 flex items-center gap-2">
              <button
                ...
                @click="silentRefreshSubDepts"
                title="Refresh">
                ⟳
              </button>
              <button
                class="px-3 py-2 rounded-lg bg-[#235AA6] text-white hover:bg-[#1f4c93]"
                @click="openCreateSubDept">
                + New
              </button>
            </div>
            ...
            <div
              ...
              <table class="w-full">
                <thead class="bg-gray-100">
                  <tr>
                    <th class="px-4 py-2 text-left text-sm text-gray-600">
                      ID
                    </th>
                    <th class="px-4 py-2 text-left text-sm text-gray-600">
                      Name
                ...
                <tbody class="divide-y">
                  <tr v-for="row in subDepts" :key="row.__key">
                    ...
                    <td class="px-4 py-2 text-sm">
                      <div class="flex items-center gap-2">
                        <button
                          class="inline-flex items-center justify-center rounded-md border border-gray-200 text-gray-600 hover:bg-gray-50 px-2.5 py-1.5"
                          title="View"
                          @click="openViewSubDept(row)">
                          <Eye class="size-4" />
                        </button>
                        <button
                          class="inline-flex items-center justify-center rounded-md border border-blue-200 text-blue-600 hover:bg-blue-50 px-2.5 py-1.5"
                          title="Edit"
                          @click="openEditSubDept(row)">
                          <Pencil class="size-4" />
                        </button>
                        <button
                          type="button"
                          ...
                          :disabled="
                            deleting &&
                            String(confirmRow?.id) === String(row.id)
                          ">
                          <Trash2 class="size-4" />
                        </button>
                      </div>
        ...
        <div
          ...
          <button
            class="px-3 py-2 rounded-lg border bg-red-600 text-white hover:bg-red-700"
            @click="editMode ? cancelEdit() : onClose()">
            {{ editMode ? "Cancel" : "Close" }}
          </button>
          <button
            v-if="!editMode"
            class="px-3 py-2 rounded-lg border bg-[#235AA6] text-white hover:bg-[#1f4c93]"
            @click="enableEdit">
            Edit
          </button>
          <button
            v-if="editMode"
            class="px-3 py-2 rounded-lg bg-[#235AA6] text-white hover:bg-[#1f4c93] disabled:opacity-60"
            :disabled="savingDept"
      ...
      <transition name="fade">
        <div
          ...
          @keydown.esc.prevent="confirming = false">
          <div
            class="absolute inset-0 bg-black/60"
            @click="confirming = false" />
          <div
            class="relative bg-white w-full max-w-md rounded-xl shadow-xl overflow-hidden">
...
function openCreateSubDept() {
  router.replace({ query: { ...route.query, sub: "create" } });
}
function openEditSubDept(row) {
  router.replace({ query: { ...route.query, sub: "edit", subId: row.id } });
}
function openViewSubDept(row) {
  router.replace({ query: { ...route.query, sub: "view", subId: row.id } });
}
...
function copyIntoForm(d) {
  const headId = d?.head_of_department?.id
    ? Number(d.head_of_department.id)
    : d?.department_head_id != null
    ? Number(d.department_head_id)
    : "";
...
async function saveDepartment() {
  ...
  if (!changedCore.value && !changedHod.value) {
    ...
    emit(
      "saved",
      buildEmitPayload(detail.value, filteredHods.value, form.hodUserId)
    );
    ...
    subDepts.value = [
      {
        ...
        active: true,
        __key: `${payload.id ?? "temp"}-${Math.random()
          .toString(36)
          .slice(2, 7)}`,
      },
      ...subDepts.value,
...
function onKey(e) {
  if (e.key === "Escape") editMode.value ? cancelEdit() : onClose();
}
