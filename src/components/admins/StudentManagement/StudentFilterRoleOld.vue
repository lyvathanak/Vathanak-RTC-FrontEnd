...
const filterKey = computed(() => {
  return `filter-${loadingData.value ? "loading" : "loaded"}-${
    departments.value.length
  }-${allPrograms.value.length}-${allSections.value.length}-${
    departmentFilterOptions.value.length
  }`;
});
...
watch(
  () => currentFilters.value.department,
  (newDept, oldDept) => {
    if (newDept !== oldDept) {
      ...
      const selectedDept = departments.value.find(
        (d) => (d.department_name || d.name) === newDept
      );
