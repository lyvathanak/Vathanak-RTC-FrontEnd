  ...
  <form @submit.prevent="submit" class="max-w-3xl mx-auto">
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div class="flex flex-col">
        ...
        <select
          ...
          required>
          <option value="">None</option>
          <option v-if="loadingDepartments" disabled value="">
            Loading departments…
          </option>
      ...
      <div class="flex flex-col" v-if="form.department_id">
        <label class="text-sm font-medium mb-1"
          >Sub-department <span class="text-red-600">*</span></label
        >
        <select
          ...
          required>
          <option value="">None</option>
          <option v-if="loadingSubDepartments" disabled value="">
        ...
        <p v-if="errors.sub_department_id" class="text-red-600 text-sm mt-1">
    ...
    <div class="mt-6 flex items-center gap-3">
      ...
      <p
        v-if="serverMessage"
        class="text-sm ml-2"
        :class="serverOk ? 'text-green-700' : 'text-red-600'">
        {{ serverMessage }}
      </p>
    </div>
  </form>
...
async function fetchDepartments() {
  ...
  try {
    ...
    departments.value = Array.isArray(res?.data?.all_department)
      ? res.data.all_department
      : [];
  } catch (err) {
...
async function fetchSubDepartments(deptId) {
  ...
  try {
    ...
    subDepartments.value = Array.isArray(res?.data?.all_sub_departments)
      ? res.data.all_sub_departments
      : [];
...
async function submit() {
  ...
  try {
    const res = await axios.post(
      "/api/managements/create_new_program",
      payload
    );
