    ...
    <div class="space-y-8">
      ...
      <section
        ...
        <div class="px-4 py-4">
            ...
            <div class="px-4 py-4 space-y-6">
              <div class="grid grid-cols-1 md:grid-cols-6 gap-3">
                ...
                <div class="flex flex-col md:col-span-3">
                  <label class="text-xs text-gray-700 mb-1"
                    >Semester Name</label
                  >
                  <input
          ...
          <!-- Loading / empty -->
          <div v-if="loadingSemesters" class="text-sm text-gray-500">
            Loading semesters…
          </div>
          <div
          ...
          <div
            ...
            <div class="px-4 py-3">
              <div class="flex items-start justify-between gap-3">
                ...
                <div
                  v-else
                  class="grid grid-cols-1 md:grid-cols-6 gap-3 w-full">
              ...
              <p
                v-if="sem._msgHead"
                class="mt-2 text-xs"
                :class="sem._okHead ? 'text-green-700' : 'text-red-600'">
                {{ sem._msgHead }}
              </p>
            </div>
            ...
            <div class="px-4 pb-4">
              <div class="flex items-center gap-2 mb-2">
                <div class="relative">
                  <select
                    ...
                    <option value="">Select subject…</option>
                    <option
                      v-for="s in availableSubjectsForSemester(sem.id)"
                      :key="s.id"
                      :value="String(s.id)">
                      {{
                        s.subject_code
                ...
                <table class="min-w-full text-sm">
                  ...
                  <tbody>
                    ...
                    <tr
                      ...
                      <td class="px-3 py-2">
                        {{ sub.total_hours ?? sub.practice_hours ?? "—" }}
                      </td>
                      <td class="px-3 py-2">
              ...
              <p
                v-if="sem._msg"
                class="text-xs mt-2"
                :class="sem._ok ? 'text-green-700' : 'text-red-600'">
                {{ sem._msg }}
              </p>
            </div>
          </div>
...
const subjectCatalog = computed(() =>
  allSubjects.value.map((s) => ({
    ...
    nameWithCode: [s.subject_code, s.subject_name || s.name]
      .filter(Boolean)
      .join(" — "),
    credit: s.credit ?? 0,
...
function startNewSemester() {
  const nextNo =
    Math.max(0, ...semesters.value.map((s) => Number(s.semester_number) || 0)) +
    1;
  newSem.value = {
    semester_number: nextNo,
    semester_key: `Semester ${nextNo}`,
    start_date: "",
    end_date: "",
    rows: [],
  };
...
function addNewRow() {
  newSem.value.rows.push({ subjectId: "", name: "", credit: null, hour: null });
}
function removeNewRow(i) {
  newSem.value.rows.splice(i, 1);
}
function onNewSubjectPicked(index, value) {
  const r = newSem.value.rows[index];
  if (!r) return;
  r.subjectId = value || "";
  const found = subjectCatalog.value.find(
...
async function createNewSemester() {
  ...
  try {
    const body = {
      ...
      semester_key: String(
        newSem.value.semester_key || `Semester ${newSem.value.semester_number}`
      ).trim(),
...
/* =========================== Subjects add / remove ========================== */
function rmKey(semesterId, subjectId) {
  return `${semesterId}|${subjectId}`;
}
function isRemoving(semesterId, subjectId) {
  return removingSet.value.has(rmKey(semesterId, subjectId));
}
...
function availableSubjectsForSemester(semesterId) {
  const present = new Set(
    (semesterSubjects[semesterId] || []).map((s) => String(s.id))
  );
...
async function loadSubjectsInSemester(semesterId) {
  ...
  try {
    ...
    semesterSubjects[semesterId] = Array.isArray(data?.subjects)
      ? data.subjects
      : [];
  } catch (e) {
