import { year } from "vue-cal/dist/i18n/ar.cjs";
...
class TimeTableAPI {
    async fetchTimeTable(page) {
        try {
            ...
            // API returns paginated data: { time_tables: { data: [...] } }
    ...
    // time slots
    // async fetchTimeSlot(per_page,year_label,year,month,day,program_id,semester_id,group_id,day_of_month){
    //     try{
    //    const res = await api.get(`/time_table/get_all_time_slots?page=${per_page}&year_label=${year_label}&year=${year}&month=${month}&day=${day}&program_id=${program_id}&semester_id=${semester_id}&group_id=${group_id}&day_of_month=${day_of_month}`)
    //    const result = res.time_slots.data;
    //    return result;
    //    console.log("result of time slots:", result);
    //     }catch(e){
    ...
    //   slots: [
    //     {
    //       time_slot: { start_time: 'HH:mm:ss', end_time: 'HH:mm:ss' },
    //       teacher_id: number|null,
    //       subject_id: number|null,
    //       location_id: number|null,
    //       remark: string|null,
    //       time_slot_date: 'YYYY-MM-DD'
    //     }
    //   ]
    // }
